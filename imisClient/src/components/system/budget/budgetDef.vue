<template>
  <div>
    ...
    <div style="text-align: left;margin-top: 10px">
      <el-button type="danger" size="mini" v-if="bgDefData.length>0" :disabled="multipleSelection.length==0"
    ...
    <div style="text-align: left">
      <el-dialog
        ...
        <el-row>
          ...
          <el-col   :span="12">
            <el-form-item label="总数计算公式:" prop="amountFomula">
              <el-input v-model="costItemDef.amountFomula" size="mini" placeholder="总数计算公式..." style="width:150px;"></el-input>
            </el-form-item>
          </el-col>
          </el-row>
          <el-row>
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            if (this.costItemDef.id) {
              ...
              this.putRequest("/system/budget/costItem", this.costItemDef).then(resp=> {
              ...
              //添加
              this.tableLoading = true;
              this.postRequest("/system/budget/costItem", this.costItemDef).then(resp=> {
                _this.tableLoading = false;
                if (resp && resp.status == 200) {
                  var data = resp.data;
                  _this.$message({type: data.status, message: data.msg});
...
handleSelectionChange(val) {
      ...
      loadTableData(){
        ...
        this.getRequest("/system/budget/settings").then(resp=> {
          _this.loading = false;
          if (resp && resp.status == 200) {
            _this.bgDefData = resp.data.costItems;
    ...
    data(){
      return {
        ...
        rules: {
          name: [{required: true, message: '必填:费用定义名称', trigger: 'blur'}],
