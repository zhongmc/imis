    ...
    <div>
      ...
      <div style="text-align: left">
        <el-dialog
          ...
          <div>
            <span>上级部门</span>
            <el-select v-model="pDepId" style="width: 200px" placeholder="请选择" size="mini">
              <el-option
...
export default {
  ...
  methods: {
    ...
    setDataToTree(treeData, pId, respData) {
      if (pId == null || pId == -1) {
        _this.treeData = treeData.concat(respData);
        return;
      }
    ...
    addDep() {
      ...
      this.postRequest("/system/basic/dep", {
        ...
        if (resp && resp.status == 200) {
          ...
          _this.setDataToTree(_this.treeData, _this.pDepId, respData.retObject);
        }
      });
    },
    loadAllDeps() {
